...
class Service_Legalios_Example_03_Social_06_FactorEmployeeReduceTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestDecExamples("03_Social_06_FactorEmployeeReduce.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            ...
            assert(expDecimal == props.factorEmployeeReduce)
          }
        }
      }
    }
