...
class Service_Legalios_Example_04_Taxing_20_MarginIncomeOfWthEmpTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("04_Taxing_20_MarginIncomeOfWthEmp.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.marginIncomeOfWthEmp)
          }
        }
      }
    }
