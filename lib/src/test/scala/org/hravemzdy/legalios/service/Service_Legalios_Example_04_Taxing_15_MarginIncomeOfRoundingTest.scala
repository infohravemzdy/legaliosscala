...
class Service_Legalios_Example_04_Taxing_15_MarginIncomeOfRoundingTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("04_Taxing_15_MarginIncomeOfRounding.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.title}") {
          ...
          it(s"GetProps should return value = ${tt.expected}") {
            assert(tt.expected == props.marginIncomeOfRounding)
          }
        }
      }
    }
