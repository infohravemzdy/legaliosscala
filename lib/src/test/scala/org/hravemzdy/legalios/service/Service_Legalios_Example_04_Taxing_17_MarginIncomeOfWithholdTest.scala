...
class Service_Legalios_Example_04_Taxing_17_MarginIncomeOfWithholdTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("04_Taxing_17_MarginIncomeOfWithhold.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.marginIncomeOfWithhold)
          }
        }
      }
    }
