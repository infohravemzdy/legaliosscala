...
class Service_Legalios_Example_02_Salary_03_MinMonthlyWageTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("02_Salary_03_MinMonthlyWage.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.minMonthlyWage)
          }
        }
      }
    }
