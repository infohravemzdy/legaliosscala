...
class Service_Legalios_Example_01_Health_03_LimMonthlyStateTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("01_Health_03_LimMonthlyState.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.limMonthlyState)
          }
        }
      }
    }
