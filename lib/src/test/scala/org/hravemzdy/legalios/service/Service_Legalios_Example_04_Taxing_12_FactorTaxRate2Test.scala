...
class Service_Legalios_Example_04_Taxing_12_FactorTaxRate2Test extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestDecExamples("04_Taxing_12_FactorTaxRate2.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            ...
            assert(expDecimal == props.factorTaxRate2)
          }
        }
      }
    }
