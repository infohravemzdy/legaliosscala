...
class Service_Legalios_Example_01_Health_07_MarginIncomeEmpTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("01_Health_07_MarginIncomeEmp.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.marginIncomeEmp)
          }
        }
      }
    }
