...
class Service_Legalios_Example_02_Salary_02_WorkingShiftTimeTest extends AnyFunSpec {
  val testList = List(
  ...
  ServiceExampleBase.logTestIntExamples("02_Salary_02_WorkingShiftTime.txt", testList)
  testList.foreach { tx =>
      ...
      tx.tests.foreach { tt =>
        describe(s"period ${tt.testName}") {
          ...
          it(s"GetProps should return value = ${tt.resultValue}") {
            assert(tt.resultValue == props.workingShiftTime)
          }
        }
      }
    }
