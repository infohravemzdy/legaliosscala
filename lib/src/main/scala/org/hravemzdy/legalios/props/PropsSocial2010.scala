...
class PropsSocial2010(version: VersionId,
  ...
  override def hasIncomeCumulatedParticy(term: WorkSocialTerms): Boolean = {
    val particy = term match {
      ...
      case WorkSocialTerms.SOCIAL_TERM_AGREEM_TASK => false
      case WorkSocialTerms.SOCIAL_TERM_SMALLS_EMPL => false
...
object PropsSocial2010 {
  def empty(): IPropsSocial = {
    return new PropsSocial2010(PropsBase.VERSION_ZERO)
  }
}
